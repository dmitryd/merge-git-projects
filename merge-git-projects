...
class Merge {
    ...
    protected function emergencyShell() {
        echo "Something went wrong. Brining the emergency shell to correct errors manually...\n";
        echo "===========================================\n\n";
    ...
    protected function mergeProject($projectName) {
        ...
        $this->executeShellCommand('git merge %s/%s --allow-unrelated-histories', array($projectName, $project['mainBranch']));
        foreach ($project['copyBranches'] as $branchName => $commitId) {
            ...
            $this->executeShellCommand('git merge %s/%s --allow-unrelated-histories', array($projectName, $branchName));
        }
